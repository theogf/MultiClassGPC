...
#
epMGPCInternal <- function(X, Y, m, Xbar_ini = NULL, log_sigma = rep(0, length(levels(Y))), autotuning = TRUE, indpoints = FALSE,
...
REPORT <- TRUE
  ...
  while (i < max_iters) {
    ...
    while(update_correct != TRUE) {
      ...
      # aNew <<- process_likelihood_factors(a, damping_inner,autotuning,indpoints)
      tryCatch(aNew <<- process_likelihood_factors(a, damping_inner,autotuning,indpoints), error = function(x) error <<- TRUE)
    ...
    change <- max(abs(aOld$f1Hat$Sigma - a$f1Hat$Sigma))
    change <- max(change, abs(aOld$f1Hat$Mu - a$f1Hat$Mu))
...
#       convergence <- TRUE
  ...
  grad_prior <- computeGradsLogNorm(a,TRUE)
  grad_likelihood <- computeGradsLikelihood(a,TRUE)
  ...
  list(log_table=value_log[,],f1Hat = a$f1Hat, logZ = logZ, grad = grad, gMGPCinfo = a$gMGPCinfo,
      n = n, nK = nK, Y = Y, a = a, levelsY = levelsY)
}
...
process_likelihood_factors <- function(a, damping, autotuning, opt_indpoints) {
  ...
  if(autotuning){
  a$gMGPCinfo <- optimize(a,opt_indpoints)}
  a
}
...
cavityDistribution <- function(a, q){
    ...
    factors_per_class <- (length(a$Y) - table(a$Y)) + table(a$Y) * (a$nK - 1)
...
computeGradsLogNorm <- function(a,opt_indpoints) {
...
computeGradsLikelihood <- function(a,opt_indpoints) {
...
optimize <- function(a,opt_indpoints) {
  ...
  grad_prior <- computeGradsLogNorm(a,opt_indpoints)
  grad_likelihood <- computeGradsLikelihood(a,opt_indpoints)
