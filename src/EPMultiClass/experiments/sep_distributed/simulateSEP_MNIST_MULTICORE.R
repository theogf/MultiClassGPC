source("../../ep_code/sep_multicore.R")
...
time <- system.time(spgpc <- epGPCInternal(data$Xtr, data$Ytr, n_pseudo_inputs = 200, eps = 1e-3, nNodes = 2))
t2 <- as.double(Sys.time())

# We make predictions

prediction <- predict(data$Xts, spgpc)
