...
computeTitledDistribution <- function(Xbar, l, sigma0, sigma, nodeInfo, nNodes) {

	Kmm <- kernel(Xbar, l, sigma0, sigma)
	R <- chol(Kmm)
	KmmInv <- chol2inv(R)

	Vinv <- KmmInv
	mNew <- rep(0, nrow(Xbar))

	for (i in 1 : nNodes) {
		Vinv <- Vinv + nodeInfo[[ i ]]$approx$eta2
		mNew <- mNew + nodeInfo[[ i ]]$approx$eta1
	}
...
predict <- function(Xtest, ret) {
	...
	posterior <- computeTitledDistribution(ret$Xbar, ret$l, ret$sigma0, ret$sigma, ret$nodeInfo, ret$nNodes)

	P_new <- kernel_nm(Xtest, ret$Xbar, ret$l, ret$sigma)
	diagKnn_new <- computeDiagKernel(Xtest, ret$sigma, ret$sigma0)

	Kmm <- kernel(ret$Xbar, ret$l, ret$sigma0, ret$sigma)
	R <- cholInverse(Kmm)

	PRtR_new <- P_new %*% t(R) %*% R
...
computeGradsPrior <- function(Xbar, l, sigma0, sigma, nodeInfo, nNodes) {
...
#	}
	...
	Ml <- 0.5 * M * Kmm
	Xl <-  (Xbar / matrix(sqrt(l), nrow(Xbar), ncol(Xbar), byrow = TRUE))
...
#	}
	...
	Xbar <- (Xbar / matrix(l, nrow(Xbar), ncol(Xbar), byrow = TRUE))
	Mbar <- t(M) * (- (Kmm - diag(sigma0, m) - diag(1e-10, m)))
	gr_xbar <- - 0.5 * 2 * (Xbar * matrix(rep(1, m) %*% Mbar, m, length(l)) - Mbar %*% Xbar)
...
# This function computes the gradients of the ML approximation provided by EP once it has converged
#
computeGradsLikelihood <- function(X, Y, Xbar, l, sigma0, sigma, nodeInfo, nNodes) {


	posterior <- computeTitledDistribution(Xbar, l, sigma0, sigma, nodeInfo, nNodes)

	mNew <- posterior$mNew
	vNew <- posterior$vNew

	gr_l <- foreach (n = 1 : nNodes) %dopar% {
		Xnode <- X[ (round((n - 1) * nrow(X) / nNodes) + 1) : round(n * nrow(X) / nNodes), ]
		Ynode <- Y[ (round((n - 1) * nrow(X) / nNodes) + 1) : round(n * nrow(X) / nNodes) ]
		computeGradsLikelihoodNode(Xnode, Ynode, vNew, mNew, Xbar, l, sigma0, sigma, nodeInfo[[ n ]]$f1Hat)
	}

	if (nNodes >= 2) {
		for (n in 2 : nNodes) {
			gr_l[[ 1 ]]$gr_log_l <- gr_l[[ 1 ]]$gr_log_l  + gr_l[[ n ]]$gr_log_l 
			gr_l[[ 1 ]]$gr_log_sigma0 <- gr_l[[ 1 ]]$gr_log_sigma0  + gr_l[[ n ]]$gr_log_sigma0
			gr_l[[ 1 ]]$gr_log_sigma <- gr_l[[ 1 ]]$gr_log_sigma  + gr_l[[ n ]]$gr_log_sigma
			gr_l[[ 1 ]]$gr_xbar <- gr_l[[ 1 ]]$gr_xbar + gr_l[[ n ]]$gr_xbar
		}
	}
...
computeGradsLikelihoodNode <- function(Xnode, Ynode, vNew, mNew, Xbar, l, sigma0, sigma, f1Hat) {
...
epGPCInternal <- function(X, Y, n_pseudo_inputs, nNodes = 1, eps = 1e-2, mpi = FALSE, start = NULL) {
	...
	if (mpi == TRUE) {
		cl <- startMPIcluster(count=nNodes)
		registerDoMPI(cl)
		foreach (i = 1 : nNodes) %dopar% {
			source("../../../ep_code/epGPC_parallel_inner_nodes_probit.R")
		}
	} else
		registerDoMC(nNodes)
	...
	nodeInfo <- foreach (i = 1 : nNodes) %dopar% {

		Xnode <- X[ (round((i - 1) * nrow(X) / nNodes) + 1) : round(i * nrow(X) / nNodes), ]
		Ynode <- Y[ (round((i - 1) * nrow(X) / nNodes) + 1) : round(i * nrow(X) / nNodes) ]

		f1Hat <- list(eta1 = rep(0, nrow(Xnode)), eta2 = rep(0, nrow(Xnode))) 
		approx <- list(eta1 = rep(0, m), eta2 = matrix(0, m, m))
		
		list(f1Hat = f1Hat, approx = approx)
	}

	for (i in 1 : nNodes) {
		if (! is.null(start)) {
			nodeInfo[[ i ]]$approx <- start$nodeInfo[[ i ]]$approx
			nodeInfo[[ i ]]$f1Hat <- start$nodeInfo[[ i ]]$f1Hat
		}
	}
	...
	while (i < 250) {
		...
		while(update_correct != TRUE) {
			...
			nodeInfoNew <- foreach (n = 1 : nNodes) %dopar% {
	
				Xnode <- X[ (round((n - 1) * nrow(X) / nNodes) + 1) : round(n * nrow(X) / nNodes), ]
				Ynode <- Y[ (round((n - 1) * nrow(X) / nNodes) + 1) : round(n * nrow(X) / nNodes) ]
				gFITCinfo <- initialize_kernel_FITC(Ynode, Xnode, Xbar, sigma, sigma0, l)

				PRtRvNewRtRPt <- rowSums((gFITCinfo$PRtR %*% vNew) * gFITCinfo$PRtR)
				C1 <- (nodeInfo[[ n ]]$f1Hat$eta2^-1 - PRtRvNewRtRPt)^-1
				PRtRvOldRtRPt <- PRtRvNewRtRPt + PRtRvNewRtRPt^2 * C1
				PRtRmNew <- gFITCinfo$PRtR %*% mNew
				C2 <- nodeInfo[[ n ]]$f1Hat$eta2 * PRtRmNew - nodeInfo[[ n ]]$f1Hat$eta1
				PRtRmOld <- PRtRmNew + PRtRvOldRtRPt * C2
				...
				alpha <-  gFITCinfo$Y / sqrt(z) * exp(dnorm(gFITCinfo$Y * 
					theta / sqrt(z), 0, 1, log = TRUE) - pnorm(gFITCinfo$Y * theta / sqrt(z), log.p = TRUE))
				...
				nodeInfo[[ n ]]$f1Hat$eta1 <- (1 - damping_inner) *  nodeInfo[[ n ]]$f1Hat$eta1  + damping_inner * eta1new
				nodeInfo[[ n ]]$f1Hat$eta2 <- (1 - damping_inner) *  nodeInfo[[ n ]]$f1Hat$eta2  + damping_inner * eta2new

				eta_vectors <-  gFITCinfo$PRtR
	
				nodeInfo[[ n ]]$approx$eta2 <- t(eta_vectors) %*% (matrix(nodeInfo[[ n ]]$f1Hat$eta2, gFITCinfo$n, 
					gFITCinfo$m) * eta_vectors) 
	
				nodeInfo[[ n ]]$approx$eta1 <- colSums(matrix(nodeInfo[[ n ]]$f1Hat$eta1, gFITCinfo$n, 
					gFITCinfo$m) * eta_vectors)
			...
			result <-  optimize(X, Y, Xbar, sigma, sigma0, l, nodeInfoNew, nNodes)
	
			Xbar_new <- result$Xbar
			sigma_new <- result$sigma
			sigma0_new <- result$sigma0
			l_new <- result$l

#			evidence <- computeEvidence(X, Y, Xbar_new, sigma_new, sigma0_new, l_new, nodeInfoNew, nNodes)
...
#			} 
		...
		for (n in 1 : nNodes) {
			change <- max(abs(nodeInfoOld[[ n ]]$approx$eta1 - nodeInfo[[ n ]]$approx$eta1))
			change <- max(change, abs(nodeInfoOld[[ n ]]$approx$eta2 - nodeInfo[[ n ]]$approx$eta2))
		}
...
#		write.table(t(c(evidence, proc.time() - t0)), 
#			file = paste("./results/time_outter_", rep, ".txt", sep = ""), row.names = F, col.names = F, append = TRUE)
#		write.table(t(c(log_l, log_sigma0, log_sigma, proc.time() - t0)), file = 
#			paste("./results/params_outer_", rep, ".txt", sep = ""), row.names = F, col.names = F, append = TRUE)
...
#		gc()
	...
	logZ <- computeEvidence(X, Y, Xbar, sigma, sigma0, l, nodeInfoNew, nNodes)

	grad_prior <- computeGradsPrior(Xbar, l, sigma0, sigma, nodeInfo, nNodes)
	grad_likelihood <- computeGradsLikelihood(X, Y, Xbar, l, sigma0, sigma, nodeInfo, nNodes)
	...
	list(logZ = logZ, grad = grad, l = l, sigma0 = sigma0, sigma = sigma, X = X, Y = Y, 
		nNodes = nNodes, nodeInfo = nodeInfo, Xbar = Xbar)
}
...
computeEvidence <- function(X, Y, Xbar, sigma, sigma0, l, nodeInfo, nNodes) {
	...
	log_evidence <- foreach (k = 1 : nNodes) %dopar% {

		Xnode <- X[ (round((k - 1) * nrow(X) / nNodes) + 1) : round(k * nrow(X) / nNodes), ]
		Ynode <- Y[ (round((k - 1) * nrow(X) / nNodes) + 1) : round(k * nrow(X) / nNodes) ]
		...
		PRtRvNewRtRPt <- rowSums((gFITCinfo$PRtR %*% vNew) * gFITCinfo$PRtR)
		C1 <- (nodeInfo[[ k ]]$f1Hat$eta2^-1 - PRtRvNewRtRPt)^-1
		PRtRvOldRtRPt <- PRtRvNewRtRPt + PRtRvNewRtRPt^2 * C1
		PRtRmNew <- gFITCinfo$PRtR %*% mNew
		C2 <- nodeInfo[[ k ]]$f1Hat$eta2 * PRtRmNew - nodeInfo[[ k ]]$f1Hat$eta1
		PRtRmOld <- PRtRmNew + PRtRvOldRtRPt * C2
		...
		theta <- PRtRmOld
		logZ <-  pnorm(gFITCinfo$Y * theta / sqrt(z), log.p = TRUE) + m / 2 * log(2 * pi) + 
			0.5 * mOldVinvmOld - 0.5 * nodeInfo[[ k ]]$f1Hat$eta2 * PRtRmOld^2 + 
			0.5 * log_det_vNew - 0.5 * log(1 - nodeInfo[[ k ]]$f1Hat$eta2 * PRtRvNewRtRPt)
	...
	log_evidence <- sum(sapply(log_evidence, sum))

	m <- ncol(vNew)
	
	log_evidence <- log_evidence + m / 2 * log(2 * pi) + 0.5 * sum(mNew * (Vinv %*% mNew)) + 0.5 * log_det_vNew
	log_evidence <- log_evidence - m / 2 * log(2 * pi) - 0.5 * sum(2 * log(diag(R)))
...
optimize <- function(X, Y, Xbar, sigma, sigma0, l, nodeInfo, nNodes) {
	...
	list(Xbar = Xbar, l = l, sigma0 = sigma0, sigma = sigma)
}
