...
(X_data,y_data,DatasetName) = get_Diabetis()
...
K_starstar = GPKernel[:compute_Kdiag](X_test,X_test)
 ...
 tic()
model = gpflow.svgp[:SVGP](X, reshape((y+1)./2,(length(y),1)), kern=GPKernel, likelihood=gpflow.likelihoods[:Bernoulli](), Z=InducingPoints, minibatch_size=BatchSize)
model[:Z][:fixed]=true
