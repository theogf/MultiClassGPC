...
NC =  Dict("LogReg"=>"Linear Model","GPC"=>"SVGPC","SPGGPC"=>"X-GPC ","Accuracy"=>"Avg. Test Error","MedianL"=>"Avg. Median Neg.\n Test Log-Likelihood")
colors_m=Dict("GPC"=>Dict(16=>"#000C66",32=>"#0014B2",64=>"#001AE5",128=>"#001AE5"),"SPGGPC"=>Dict(16=>"#7F0900",32=>"#CC0F00",64=>"#FF1200",128=>"#FF524B"),"LogReg"=>"g")
colors=Dict("GPC"=>cols[1],"SPGGPC"=>cols[3],"LogReg"=>cols[2])
linestyles=Dict(8=>":",16=>"--",32=>":",64=>"-.",128=>"-")
markers=Dict(8=>"",16=>"",32=>"",64=>"",128=>"")
...
function InducingPointsComparison(metric,MPoints=[16,32,64,128];step=1)
    ...
    for M in MPoints
        ...
        for (name,res) in Results
            ...
            new_p,=semilogx(res[1:step:end,1],res[1:step:end,metrics[metric]],markersize=gmarkersize,color=colors_m[name][M],marker=markers[M],linewidth=gwidth,linestyle=linestyles[M],label=NC[name]*" M=$M")
            push!(p[name],new_p)
        end
    end


    xlabel("Training Time in Seconds",fontsize=22.0)
    ylabel(NC[metric],fontsize=22.0)
    title("Shuttle",fontsize=24.0)

    legend([p["SPGGPC"];p["GPC"]],[lab["SPGGPC"];lab["GPC"]],fontsize=20.0)
    xlim([0.3,4000])
    ylim([0.0,0.25])
    xticks(fontsize=16.0)
	yticks(fontsize=16.0)
    tight_layout()
...
function PlotMetricvsTime(dataset,metric;time=true,writing=false,corrections=true)
    Results = Dict{String,Any}();
    println("Working on dataset $dataset")
    # colors=Dict("GPC"=>"b","SPGGPC"=>"r","LogReg"=>"y")
    time_line = Dict("GPC"=>[1:1:99;100:10:999;1000:100:9999;10000:1000:100000],"SPGGPC"=>[1:1:99;100:10:999;1000:100:10000])
    p = Dict{String,Any}()
    ...
    Results["GPC"] = readdlm("../final_cluster_results/ConvergenceExperiment/ConvergenceExperiment_"*dataset*"Dataset/Results_GPC.txt")
    Results["SPGGPC"] = readdlm("../final_cluster_results/ConvergenceExperiment/ConvergenceExperiment_"*dataset*"Dataset/Results_SPGGPC.txt")
    ...
    maxx = Results["GPC"][end,1]
    minx = Results["SPGGPC"][1,1]
    if metric != "Final"
    ...
    if time
        time_line = Dict("GPC"=>Results["GPC"][:,1],"SPGGPC"=>Results["SPGGPC"][:,1])
    elseif dataset == "German" || dataset == "Diabetis"
        time_line = Dict("GPC"=>[1:1:99;100:10:999;1000:100:9999;10000:1000:50000],"SPGGPC"=>collect(1:length(Results["SPGGPC"][:,1])))
    elseif dataset == "Covtype"
        time_line = Dict("GPC"=>[1:1:99;100:10:999;1000:100:9999;10000:1000:10000],"SPPGPC"=>[1:1:99;100:10:999;1000:100:9999;10000:1000:50000])
    elseif  dataset == "SUSY" || dataset == "HIGGS"
        time_line = Dict("GPC"=>[1:1:99;100:10:999;1000:100:9999;10000:1000:100000],"SPGGPC"=>[1:1:99;100:10:999;1000:100:9999;10000:1000:50000])
    end
    if metric == "Final"
        iter=1
        giter = 2
        for (mname,mmetric) in metrics
            if in(mname,FinalMetrics)
                ...
                legend([p["SPGGPC"];p["GPC"];p["LogReg"]],[NC["SPGGPC"];NC["GPC"];NC["LogReg"]],fontsize=17.0,loc=1,fancybox=true,frameon=true,facecolor="white",framealpha=0.7)
                giter-=1
            end
        end
	
	suptitle(dataset,fontsize=24.0)
    elseif metric != "All"
        println(name)
	...
	println("Saved under figure in PGGPC/Plots"*(metric=="Final"?"Final":"")*"Convergence_vs_"*(time?"time":"iterations")*"_on_"*dataset*".png")
        close()
    end
end
