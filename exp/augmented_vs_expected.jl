...
t_full = @elapsed emodel.train(iterations=1000,callback=callback2)

global y_full = emodel.predictproba(X_test)
global y_fall, = AugmentedGaussianProcesses.multiclasspredict(emodel,X_test,true)
...
pemetrics = plot(emetrics,title="Expected")
pametrics = plot(ametrics,title="Augmented")
metlims = (min(ylims(pemetrics)[1],ylims(pametrics)[1]),max(ylims(pemetrics)[2],ylims(pametrics)[2]))
pmet = plot(plot(emetrics,title="Expected",ylims=metlims),plot(ametrics,title="Augmented",ylims=metlims))
pekerparams = plot(ekerparams,title="Expected",yaxis=:log)
pakerparams = plot(akerparams,title="Augmented",yaxis=:log)
parlims = (min(ylims(pekerparams)[1],ylims(pakerparams)[1]),max(ylims(pekerparams)[2],ylims(pakerparams)[2]))
pker = plot(plot(ekerparams,title="Expected",t=:line,yaxis=:log,ylims=parlims),plot(akerparams,title="Augmented",yaxis=:log,ylims=parlims))
