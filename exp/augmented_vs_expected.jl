...
pakerparams = plot(akerparams,title="Augmented")
parlims = (min(ylims(pekerparams)[1],ylims(pakerparams)[1]),max(ylims(pekerparams)[2],ylims(pakerparams)[2]))
pker = plot(plot(ekerparams,title="Expected",yaxis=:log,ylims=parlims),plot(akerparams,title="Augmented",yaxis=:log,ylims=parlims))
